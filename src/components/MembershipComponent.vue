...
<template>
  <v-card v-if="winner">
    <v-card-item :class="getStatusColor(winner)">
      ...
      <v-row
        class="bg-black mt-1"
        v-if="winner.membresia.fechaInicio && winner.membresia.fechaFin"
      >
        <v-col cols="6">
          <v-text-field
            ...
            label="Día inicio"
            readonly
            :model-value="winner.membresia.fechaInicio"
            variant="solo"
          >
          </v-text-field>
        </v-col>
        <v-col cols="6">
          <v-text-field
            ...
            label="Último día"
            readonly
            :model-value="winner.membresia.fechaFin"
            variant="solo"
          >
          </v-text-field>
        </v-col>
    ...
    <v-card-text>
      <v-tabs-window v-model="tabSelected">
        <v-tabs-window-item value="Promociones">
          ...
          <v-row
            v-for="promocion in winner.membresia.promocionesAdquiridas"
            :key="promocion.idConcepto"
            :title="promocion.idConcepto"
            prepend-icon="mdi-star"
            :subtitle="promocion.fechaCreacion"
          >
            <v-col cols="12">
