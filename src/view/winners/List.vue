...
function getColorFrom(membresia: Membership): string {
    ...
    let estado = MembershipsStateEnum[estadoString as keyof typeof MembershipsStateEnum]
...
<template>
    ...
    <v-progress-circular :size="50" color="amber" indeterminate v-if="isLoadingSetWinners"></v-progress-circular>
                ...
                <v-table v-if="!isLoadingSetWinners && winners" hover>
                    <thead class="bg-grey-darken-2">
                        <tr>
                            ...
                            <th class="text-left">
                                Estado actual membresia
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <template v-if="winners">
                            <tr v-for="user in winnerList" :key="user.id" @click="clickableRow(user)"
                                ...
                                <td>
                                    <template v-if="user.membresia">
                                        <v-chip :color="getColorFrom(user.membresia)" variant="flat">
